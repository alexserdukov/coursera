...
trait WikipediaApi {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] =
      Observable.create[Try[T]](obr => {
        obs.subscribe(new Observer[T] {
          override def onNext(value: T) = obr.onNext(Success(value))
          override def onError(error: Throwable) = {
            obr.onNext(Failure(error))
            obr.onCompleted()
          }
          override def onCompleted() = obr.onCompleted()
        })
      })
